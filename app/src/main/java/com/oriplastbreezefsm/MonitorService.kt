...
import com.oriplastbreezefsm.app.domain.GpsStatusEntity
import com.oriplastbreezefsm.app.domain.NewGpsStatusEntity
import com.oriplastbreezefsm.app.domain.PerformanceEntity
...
import com.oriplastbreezefsm.features.location.LocationJobService
...
//        return super.onStartCommand(intent, flags, startId)
    ...
    fun shouldServiceUpdate(): Boolean {
        return if (Math.abs(System.currentTimeMillis() - Pref.prevServiceTimeStampMonitorService) > 3000) {
            Pref.prevServiceTimeStampMonitorService = System.currentTimeMillis()
            true
            //server timestamp is within 5 minutes of current system time
        } else {
            false
    ...
    fun restartLocationService() {
        try {
            if(Pref.IsLeavePressed== true && Pref.IsLeaveGPSTrack == false){
                Timber.d("MonitorService restartLocationService return Time :" + AppUtils.getCurrentDateTime())
                return
            }
            val serviceLauncher = Intent(this, LocationFuzedService::class.java)
            Timber.d("MonitorService restartLocationService Time :" + AppUtils.getCurrentDateTime())
